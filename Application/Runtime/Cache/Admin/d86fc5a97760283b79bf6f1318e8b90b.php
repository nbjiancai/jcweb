...
<script type="text/javascript">
    ...
    <div id="page-inner" class="container">
        <ul class="nav nav-tabs">
            <li role="presentation" class="active" id="de_A" name="de"><a href="/admin/shengedit/information?de=A" >动态资讯</a></li>
            <input type="hidden" id="de_choose" value="<?php echo ($de); ?>"/>
        </ul>
        <p></p>
        <div >

            <a class="btn btn-success btn-xs" href="<?php echo U('/admin/shengedit/addInformation');?>?de=<?php echo ($de); ?>"><i class="glyphicon glyphicon-plus"></i>新增</a><p/>

            <table class="table table-bordered table-striped table-hover">
                <thead>
                <th width="15%">展示图</th>
                <th width="50%">
                    <?php if($de == A): ?>动态资讯标题<?php endif; ?>
                </th>
                <th >操作</th>
                </thead>
                <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><tr>
                        <td ><img src="<?php echo ($one["information_images_path"]); ?>" onerror="this.src='/Public/static/images/default-timg.gif'" style="width: 90px;"></td>
                        ...
                        <td>
                            ...
                            <a class="btn btn-primary btn-xs" href="<?php echo U('/admin/shengedit/informationPicture');?>?id=<?php echo ($one["id"]); ?>&type=I" ><i class="glyphicon glyphicon-picture"></i>插入/更换产品图片</a>
                            <a class="btn btn-primary btn-xs" href="<?php echo U('/admin/shengedit/addInformation');?>?id=<?php echo ($one["id"]); ?>" ><i class="glyphicon glyphicon-cog"></i>修改内容</a>
...
<script type="text/javascript">
    ...
    function doSpecial(id){
        ...
        $.ajax({
            type:"post",
            url:"<?php echo U('/admin/shengedit/doSpecial');?>",
            data:{"id":id},
            dataType:"json",
    ...
    function doneDel(id){
        ...
        $.ajax({
            type:"post",
            url:"<?php echo U('/admin/shengedit/doInfoDelete');?>",
            data:{"id":id},
            dataType:"json",
            success:function(ret){
                if(ret.msg=='succ'){
