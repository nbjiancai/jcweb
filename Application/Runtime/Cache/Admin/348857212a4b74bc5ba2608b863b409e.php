...
<script type="text/javascript">
    ...
    <div id="page-inner" class="container">
        <ul class="nav nav-tabs">
            <li role="presentation" class="active" id="de_A" name="de"><a href="/admin/shengedit/production?de=A" >高性能外延材料与器件</a></li>
            <li role="presentation" id="de_B" name="de"><a href="/admin/shengedit/production?de=B">专用设备服务</a></li>
            <input type="hidden" id="de_choose" value="<?php echo ($de); ?>"/>
        </ul>
        <p></p>

            <a href="<?php echo U('/admin/shengedit/addProduction');?>?de=<?php echo ($de); ?>" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-plus" ></i>新增</a>
            <p></p>
            <div>
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                <a class="btn btn-primary btn-xs" href="<?php echo U('/admin/shengedit/informationPicture');?>?id=<?php echo ($one["id"]); ?>&type=P" ><i class="glyphicon glyphicon-picture"></i>插入/更换展示图片</a>
                                <a href="<?php echo U('/admin/shengedit/addProduction');?>?id=<?php echo ($one["id"]); ?>&de=<?php echo ($one["type"]); ?>" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-edit"></i> 编辑</a>
                                <a href="javascript:void(0);" onclick="onDelete('<?php echo ($one["id"]); ?>')" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-trash"></i> 删除</a>
                            </td>
...
<script type="text/javascript">
    ...
    function doneDel(id){
        ...
        $.ajax({
            type:"post",
            url:"<?php echo U('/admin/shengedit/doProductionDelete');?>",
            data:{"id":id},
            dataType:"json",
