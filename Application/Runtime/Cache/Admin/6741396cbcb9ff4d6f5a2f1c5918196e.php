			...
			<table class="table table-bordered table-striped table-hover">
				...
				<tbody>
					<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><tr>
					   		...
					   		<td><?php if($one['super_admin']==1){echo '是';}else{echo '否';} ?></td>
					   		<td><?php echo ($one["rolename"]); ?></td>
					   		<td><?php if($one['status']==1){echo '正常';}else{echo '已禁用';} ?></td>
					   		<td><?php echo ($one["login_time"]); ?></td>
					   		<td>
					   			<span onclick="editAdminUser(<?php echo ($one["id"]); ?>)" class="glyphicon glyphicon-pencil">修改</span>
					   		</td>
...
function editAdminUser(id){
	...
	$.ajax({
		type:"post",
		url:"<?php echo U('/admin/auth/getAdminUser');?>",
		dataType:"json",
		data:{"id":id},
...
function doAddAdminUser(){
	var options = {
    	url: "<?php echo U('/admin/auth/doAddAdmin');?>",
    	dataType: 'json',
